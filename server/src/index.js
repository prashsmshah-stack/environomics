...
function getStartupSummary() {
  return {
    ...
    storageDriverConfigured: process.env.STORAGE_DRIVER ?? null,
    ...
    adminPasswordAutoGenerated: env.adminPasswordWasAutoGenerated,
    adminTokenSecretAutoGenerated: env.adminTokenSecretWasAutoGenerated
  };
}
...
async function initializeContentStorageWithFallback() {
  try {
    await initializeContentStorage();
  } catch (error) {
    if (env.storageDriver !== "mysql") {
      throw error;
    }

    console.error("Primary MySQL storage initialization failed:", error);
    console.error("Storage fallback: switching to JSON file storage for this boot.");
    await closeMysqlPool().catch(() => {});
    env.storageDriver = "json";
    await initializeContentStorage();
  }
}
...
async function start() {
  validateRuntimeEnv();
  await initializeContentStorageWithFallback();
