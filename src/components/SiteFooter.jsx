...
export default function SiteFooter() {
  ...
  const settings = useMemo(
    () => ({
      ...fallbackSettings,
      ...(content?.settings ?? {}),
      footerYear: footer.year || content?.settings?.footerYear || fallbackSettings.footerYear,
    }),
    [content, footer.year]
  );
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-10 grid grid-cols-1 gap-10 md:grid-cols-2 xl:grid-cols-4">
          <div>
            <div className="mb-8 flex items-center gap-3 sm:mb-10">
              <img
                src={resolveMediaUrl(footer.logo?.url ?? footer.logo) || footerLogo}
          ...
          <div>
            ...
            <ul className="helixa-regular space-y-4 text-sm text-white/80">
              {(Array.isArray(footer.services) && footer.services.length ? footer.services : footerServices).map((item) => (
                <li key={item.label}>
          ...
          <div>
            ...
            <ul className="helixa-regular space-y-4 text-sm text-white/80">
              {(Array.isArray(footer.quickLinks) && footer.quickLinks.length ? footer.quickLinks : footerLinks).map((item) => (
                <li key={item.label}>
                  <a href={item.href} className="transition-colors hover:text-white">
                    {item.label}
                  </a>
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-8 text-center sm:gap-6 sm:pt-10 md:flex-row md:text-left">
          ...
          <div className="helixa-bold flex flex-wrap justify-center gap-4 text-[11px] uppercase tracking-[0.25em] text-white/40 sm:gap-8 md:justify-end lg:gap-12">
            {(Array.isArray(footer.bottomWords) && footer.bottomWords.length
              ? footer.bottomWords.map((item) => item.label)
