...
function getManagedMediaUrl(value, fallback = "") {
  return resolveMediaUrl(value?.url ?? value) || fallback;
}
...
export default function HomePage() {
  ...
  const rawHomeContent = content?.home ?? {};
  const homeContent = rawHomeContent
    ? { ...defaultHomeContent, ...rawHomeContent }
    : defaultHomeContent;
  const managedHeroImage = getManagedMediaUrl(homeContent.heroImage, heroBackgroundImage);
  const managedHeroImageSmall = getManagedMediaUrl(homeContent.heroImageMobile, heroBackgroundImageSmall);
  const managedAboutImage = getManagedMediaUrl(homeContent.aboutImage, aboutEnvironomicsImage);
  const managedImpactCards =
    Array.isArray(homeContent.impactCards) && homeContent.impactCards.length
      ? homeContent.impactCards.map((card, index) => ({
          ...impactCards[index % impactCards.length],
          ...card,
        }))
      : impactCards;
  const managedFeaturedVideos =
    Array.isArray(homeContent.featuredVideos) && homeContent.featuredVideos.length
      ? homeContent.featuredVideos.map((video, index) => {
          const youtubeUrl = String(video.youtubeUrl ?? "").trim();
          const youtubeId =
            youtubeUrl.match(/(?:youtu\.be\/|v=|embed\/)([a-zA-Z0-9_-]+)/)?.[1] ?? "";

          return {
            ...featuredVideos[index % featuredVideos.length],
            ...video,
            id: video.id ?? `featured-video-${index}`,
            kind: video.kind === "youtube" ? "embed" : video.kind,
            embedUrl: youtubeId
              ? `https://www.youtube-nocookie.com/embed/${youtubeId}?rel=0`
              : video.embedUrl,
            thumbnailUrl:
              getManagedMediaUrl(video.thumbnail) ||
              (youtubeId ? `https://i.ytimg.com/vi/${youtubeId}/maxresdefault.jpg` : video.thumbnailUrl),
            watchUrl: youtubeUrl || video.watchUrl,
          };
        })
      : featuredVideos;
  const managedServices =
    Array.isArray(homeContent.services) && homeContent.services.length
      ? homeContent.services.map((service, index) => ({
          ...services[index % services.length],
          ...service,
          image: getManagedMediaUrl(service.image, services[index % services.length]?.image),
          alt: service.alt || service.title,
        }))
      : services;
  const criticalImages = useMemo(() => [
    managedHeroImage,
    managedHeroImageSmall,
    managedAboutImage,
    ...managedServices.map((service) => service.image).filter(Boolean),
  ], [managedAboutImage, managedHeroImage, managedHeroImageSmall, managedServices]);
      ...
      <section className="hero-split-gradient relative flex min-h-[85svh] items-center overflow-hidden pt-20 lg:min-h-[90vh]">
        <div className="hero-media-layer absolute inset-0" aria-hidden="true">
          <img
            ...
            srcSet={`${managedHeroImageSmall} 1600w, ${managedHeroImage} 2560w`}
            sizes="100vw"
            alt=""
        ...
        <div className="mx-auto flex w-full max-w-[1440px] justify-center px-4 sm:px-6 lg:px-8 xl:px-12">
          <div className="relative z-10 flex max-w-5xl flex-col items-center py-12 text-center sm:py-16">
            ...
            <div className="flex w-full flex-col justify-center gap-4 sm:w-auto sm:flex-row">
              ...
              >
                <span className="block max-w-full" style={singleLineClampStyle} title="Our Culture">
                  Our Culture
                </span>
        ...
        <section className="bg-transparent pb-16 pt-12 sm:pb-20 sm:pt-14 lg:pb-24">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
              {managedFeaturedVideos.map((video) =>
                video.kind === "embed" ? (
                  <article
                    key={video.id}
