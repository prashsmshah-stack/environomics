...
export default function ContactUsPage() {
  ...
  const contact = useMemo(() => {
    const backendContact = contactPage;
  ...
  const urgentInquiry = contactPage.urgentInquiry ?? {};
  const inquirySettings = contactPage.inquiryForm ?? {};
  const facility = contactPage.facility ?? {};
  const managedServiceOptions =
    Array.isArray(inquirySettings.serviceOptions) && inquirySettings.serviceOptions.length
      ? inquirySettings.serviceOptions.map((item) => item.label).filter(Boolean)
      : serviceOptions;
  const managedInquiryFields =
    Array.isArray(inquirySettings.fields) && inquirySettings.fields.length
      ? inquirySettings.fields
      : inquiryFieldDefinitions;
  ...
  const mapHref = useMemo(
    () => getMapUrl(facility.mapDestination || facility.address || contact.address),
    [contact.address, facility.address, facility.mapDestination]
  );
  const ctaCards = useMemo(
    () => (Array.isArray(contactPage.ctaCards) && contactPage.ctaCards.length ? contactPage.ctaCards : [
      {
        eyebrow: "Assessment",
      ...
      ...card,
      href: /whatsapp/i.test(card.title ?? "") ? whatsappHref : card.href,
      color: /whatsapp/i.test(card.title ?? "") ? "text-[#25D366]" : "text-primary",
      extraClass: /whatsapp/i.test(card.title ?? "") ? "cta-whatsapp" : "",
      titleHoverColor: /whatsapp/i.test(card.title ?? "") ? "group-hover:text-[#25D366]" : "group-hover:text-primary",
    })),
          ...
          <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
            <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-3 lg:gap-16">
              <div id="contact-inquiry-form" className="stagger-item scroll-mt-28 rounded-2xl border border-outline-variant/30 bg-white p-6 shadow-sm sm:p-8 md:p-10 lg:col-span-2 lg:p-12" style={{ animationDelay: "0.4s" }}>
                <div className="mb-10">
                  <h2 className="font-optika-bold mb-2 text-3xl text-deep-navy">{inquirySettings.title || "Inquiry Form"}</h2>
                  <p className="font-helixa-regular text-sm text-slate-500">{inquirySettings.subtitle || "Provide your technical requirements below for a detailed assessment."}</p>
                </div>
                {submitError ? (
                ...
                <form className="grid grid-cols-1 gap-x-8 gap-y-6 md:grid-cols-2" onSubmit={handleInquirySubmit}>
                  {managedInquiryFields.map((field) => (
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    >
                      {managedServiceOptions.map((item) => <option key={item}>{item}</option>)}
                    </select>
                  </div>
                  <div className="flex flex-col gap-2">
                  ...
                  <div className="pt-6 md:col-span-2">
                    ...
                    >
                      {isSubmitting ? "Submitting Inquiry..." : inquirySettings.submitLabel || "Submit Technical Inquiry"}
              ...
              <div className="space-y-6">
                <div className="contact-card-click rounded-2xl border-2 border-primary/20 bg-white p-6 shadow-md transition-all hover:border-primary/40 sm:p-8">
                  <div className="mb-6 flex items-start justify-between">
                    <span className="font-helixa-bold rounded bg-primary px-3 py-1 text-[9px] uppercase tracking-widest text-white">{facility.badge || "Corporate Headquarters"}</span>
                    <Icon name="corporate_fare" className="text-3xl text-primary" />
                  </div>
                  <h3 className="font-optika-bold mb-5 text-2xl text-deep-navy">{facility.officeTitle || "Registered Office"}</h3>
                  <div className="mb-6 flex items-start gap-4">
                    ...
                    <div className="font-helixa-regular min-h-[6.5rem] text-sm leading-relaxed text-slate-600">
                      <p className="font-helixa-bold mb-1 text-deep-navy">{facility.companyName || "Environomics Projects LLP"}</p>
                      {(facility.address ? getAddressLines(facility.address) : addressLines).length ? (
                        (facility.address ? getAddressLines(facility.address) : addressLines).map((line) => (
                  ...
                  <div className="flex items-center gap-4 border-t border-outline-variant/20 pt-6">
                    ...
                    <p className="font-helixa-regular text-xs text-slate-500">{facility.hours || "Mon to Sat: 9:00 AM to 6:30 PM IST"}</p>
                  </div>
                </div>
                ...
                <div className="contact-card-click group relative overflow-hidden rounded-2xl border-none bg-primary p-6 text-white sm:p-8">
                  <div className="relative z-10">
                    <h3 className="font-optika-bold mb-3 text-xl">{facility.mapTitle || "Visit Our Facilities"}</h3>
                    <p className="font-helixa-regular mb-8 text-sm text-white/90">{facility.mapDescription || "View our engineering studio and live project monitoring centers."}</p>
        ...
        <section className="relative overflow-hidden bg-white px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
          <div className="relative z-10 mx-auto max-w-6xl">
            <div className="architectural-inquiry-box relative overflow-hidden rounded-2xl p-8 text-on-background sm:p-10 md:p-14 lg:p-16" style={{ borderLeftColor: '#2AAF6F' }}>
              ...
              <div className="relative z-10">
                <h2 className="font-optika-bold mb-6 text-3xl tracking-tight text-growth-green sm:text-4xl md:text-5xl">{urgentInquiry.title || "Urgent Inquiries?"}</h2>
                <p className="font-helixa-regular mb-10 max-w-2xl text-base text-growth-green sm:mb-12 sm:text-lg">{urgentInquiry.description || "For immediate assistance regarding project maintenance or emergency audits, connect with our 24/7 industrial support team."}</p>
                <div className="grid max-w-4xl grid-cols-1 gap-8 md:grid-cols-2 md:gap-12">
                  {[
                    { icon: "call", label: urgentInquiry.phoneLabel || "Industrial Hotline (24/7)", values: [urgentInquiry.phone || contact.phone] },
                    { icon: "inbox", label: urgentInquiry.emailLabel || "Project Support", values: parseContactEmails(urgentInquiry.email || contact.email) },
                  ].map((item) => (
